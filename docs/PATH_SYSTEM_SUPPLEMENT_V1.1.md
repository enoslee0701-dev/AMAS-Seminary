...
# D. Phase 1 的"1 次反馈"如何落地（解决时序矛盾）

V1.0 Phase 1 要求"1 门课 + 1 项实践 + 1 次反馈"，但导师系统在 Phase 4。裁决如下：

Phase 1 的"反馈"分三档，**按可得性降级**：

| 档位 | 形式 | Phase 1 是否可用 | 实现方式 |
|---|---|---|---|
| F1 自我复盘 | 用户按 reflection 字段写 100–200 字 | ✅ 可用 | 复用现有服事记录的"反思"输入 |
| F2 同工反馈 | 用户线下请一位同工看记录并口头反馈，自己录入一句话 | ✅ 可用 | 服事记录增加"他人反馈"文本字段 |
| F3 导师正式反馈 | 导师在后台观察并填写评估 | ❌ Phase 4 | 现阶段 UI 只显示「规划中」 |

**Phase 1 的"1 次反馈"= F1 必做 + F2 建议**，不得因 F3 未上线而阻塞闭环。证据等级按 V1.0 §12.2：F1→E1，F2→E2/E3，F3→E4。
...
| # | 事项 | 文件 | 类型 |
|---:|---|---|---|
| 1 | `learningBoost` 改为学习证据计数，不再加分 | `CustomTheologyView.tsx` | 行为变更 |
| 2 | 恩赐题库/匹配算法移入 `legacy/` 目录，保留 `GiftsResult` 类型 | `CustomTheologyView.tsx` | 重构 |
| 3 | 文件头注释与同步服务注释去除"诊断/恩赐辨识" | 2 处 | 措辞 |
| 4 | 课程加 `availability` 字段，21 门标 `in_development` | `catalog.ts` | 数据 |
| 5 | 课程 id 硬编码收敛到单一映射表 | 4 个文件 | 重构 |
| 6 | `GROWTH_SYSTEM.md` 正文移入 Legacy 章节 | docs | 文档 |
| 7 | 结果页课程列表改为 now/next/later 分层 | `ChristianProfileView.tsx` | UI |

**第 1、2、5 项会改变现有行为，需你确认后再动。第 3、4、6 项无风险，可先做。**
