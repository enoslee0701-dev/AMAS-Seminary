...
>
> **而且：事件投递不是无损的。** Postgres 的 IDENTITY 分配顺序 ≠ 提交顺序，
> 拿到较小 id 的事务可能在较大 id 已可见之后才提交。轮询用了 50 个 id 的
> 有界回看窗口缓解，**超出窗口的仍会丢**；`INSERT` 失败的事件更是压根不存在，
> 不可能靠续传补回。真正的兜底是客户端侧的周期性 REST 刷新与重连 full refresh。
> 详见 `docs/operations/DB-13C-REALTIME-CUTOVER-REPORT.md` 的「契约限制」一节。
>
> ### DB-13B 切换结果（当轮实测）
>
> ```
