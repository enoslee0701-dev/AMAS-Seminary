...
## #39 本机数据不按身份分桶：聊天记录、评估结果、每题作答 — `CLOSED`（2026-09-12）

```
status:    CLOSED
severity:  high（隐私：同一台设备换个人登录就看得见上一个人的私人数据）
owner:     unassigned
phase:     整合前核查（2026-09-12）
```

整合前按监督要求核「本地存储按身份读取是否会泄露他人历史」，**查出两处、
都实测复现**。加上此前的自建群聊（#30 系列），这是同一类缺陷的第三、第四处。

### 复现

```
聊天记录  amas_chat_messages
  甲登录发一条消息 → 甲登出 → 乙在同一台设备登录 → 乙看见甲的聊天记录
  那个全局键当时有三个写入方（ChatView 自己、语音房分享、校友圈分享），
  三个都不看身份

评估与成长档案  amas_ct_state_v2 / amas_cp_session_v1
  甲完成评估 → 登出 → 乙登录 → 打开「定制化神学」，界面按甲那份已完成的
  结果渲染（出现「重新评估 / 删除结果 / 倾向指数」）
...
那个请求用的是**当前登录者的 access token**。所以在配了后端的环境里，
乙登录后只要触发一次保存，本地那份（其实是甲的）就会被 PUT 到**乙的账号**下。

**这条没有实测复现** —— 需要可用后端与两个真实账号，那是不能碰的。
只记机制与前提，不声称已经发生过。按身份分键之后前提不再成立。

### 修法

新增 `services/chatMessages.ts` 与 `services/assessmentStorage.ts`，
沿用自建群聊那套已过审的纪律：

```
按身份分键；没有身份既不读也不写
写入后读回逐字节核对；落盘失败如实返回 persisted=false
**一律无损**：不截断、不按形状过滤、不因为写不下就删历史
旧的全局内容归属未知：原字节挪进隔离位，不归给任何身份、不展示，
  不按解析结果删除，隔离位已占不覆盖，写不成就让源原地不动
```

评估那边十几个读写点遍布整个流程，逐个传 userId 会改动大量与本缺陷无关的
签名、反而更容易漏，所以由 `App` 在身份变化时设一次当前身份
（`useLayoutEffect`，早于懒加载视图挂载）。**默认在安全那一侧**：
没设身份时读返回 null、写返回 false。业务逻辑一行没动。

### 途中自查出的两处「我自己造的坑」

```
1. chatMessages 第一版有 slice(-500) 和「形状不认识就过滤掉」
   读一次再存一次就悄悄删数据。产品只授权保留，没授权自动删历史。
   这跟当初那条结论是同一条：**解析器不认识 ≠ 不是真数据** ——
   我把那条规矩只用在了旧全局键的隔离上，没用在身份桶自己身上。
   现在持久层一律无损，**渲染层**才做上限与跳过（只影响画面）。
2. verify-chat-identity 有一节是空跑假绿
   「登出后看不到任何人的记录」两条都 PASS，但没有身份时应用停在登录门上，
   页面里本来就什么都没有 —— 对着一张登录页当然「看不到」。
   同一个脚本还把加载失败悄悄记进 anyFatal、屏幕上却印着 23/23 PASS，
   退出码却是 1。现在加载失败记成明明白白的 FAIL，那一节改成验真正成立的事。
```

### 回归

```
tests/services/chatMessages.test.ts          28/28（新增 fixture：无损持久化）
scripts/verify-chat-identity.mjs             24/24
scripts/verify-assessment-identity.mjs       14/14（新增）
scripts/verify-assessment-resume.mjs         18/18（探针改读分桶键；
                                               30 题续答等结论全部不变）
scripts/verify-chat-composer.mjs             65/65
vitest 327/327 · tsc 干净 · build PASS
```

两个新脚本都已接入 `npm run verify:ui-flows` 与 CI 的独立 `ui-flows` job。

### 仍未处理（记下，未做）

同一类的全局键还有若干，各自保存的私人程度不同，本轮**没有**一并动，
免得把一次整合前的核查滚成大范围改造：

```
amas_joined_groups          加入了哪些群组
amas_feed_cover             校友圈封面图
amas_downloaded_files       下载过哪些课件
amas_course_reports         课程举报记录
amas_cooperation_submissions 事工合作表单提交（含联系方式）
amas_sermon_notes_<roomId>  讲道笔记
amas_privacy                隐私设置
```

要不要一并分桶、旧数据怎么处理，建议作为一个独立条目统一裁定。

---

## #40 本机私人记录不按身份分桶（第二、三批）+ 两条「送不出去却说已提交」 — `CLOSED`（2026-09-12）

```
status:    CLOSED
severity:  high（隐私 + 诚实性；其中课程举报那条落在安全相关路径上）
owner:     unassigned
phase:     产品完善（2026-09-12）
```

接 #39 把登记在册的其余键处理完。**每一处都先真实复现再修。**

### 第二批（提交 `fa04f10`）

```
amas_cooperation_submissions  机构名 / 联系人 / 邮箱堆在同一份全局数组里
                              （这份数据全仓只有写、没有读，界面不回显 ——
                               所以问题在存储层，不是「乙看见了甲的邮箱」）
amas_sermon_notes_<roomId>    按房间分键但不按人分：换个人进同一间房，
                              笔记面板里是上一个人写的讲章
顺带                          后端没配 / 请求失败时回落本机留档，界面却显示
                              「申请已提交 …… 2 个工作日内邮件联系」
```

### 第三批（提交 `5e09834`）

先定产品语义。判据是「设备偏好可以留在设备级，私人记录按人分」，
逐个看下来**五个全是私人记录**：加入的群组、校友圈封面、下载过哪些课件、
举报过哪门课、隐私开关。真正属于设备偏好的另一类（`amas_lang`、
`amas_push_enabled`、各种「不再提示」标记）跟谁登录无关，**刻意没动**。

顺带修掉一条比较重的不实：**课程举报送不到任何人手里**。
`backend/src/routes` 里只有房间范围的代祷分享举报，没有课程举报的通道；
`amas_course_reports` 全仓只有写、没有读。界面原本却说「已收到您的反馈 ……
1–3 个工作日内核实并回复」。在一条安全相关的路径上这么说尤其不该 ——
举报有害内容的人会以为已经在处理了。现在两处都说清没有接通受理通道，
并指一条真的可用的路。

### 公共层与失败边界

这类逻辑第三次出现，抽成 `services/scopedLocalStore.ts`：按身份分键、
没有身份不读不写、写后读回逐字节核对、**只搬字节**（不解析、不截断、
不按形状过滤）、追加时已有条目一个不动、结构不认识先留底再开新的、
留不了底就拒绝写入、旧的全局内容原字节隔离且不归属、异步入口 `owner`
绑定发起者。

隔离是**在访问到那个键时**发生的，不是启动时全仓扫一遍 ——
那样等于替用户决定动哪些数据。

失败一律看得见：加群 / 换封面 / 隐私开关 / 讲道笔记写不成都会说出来。

### 回归

```
scopedLocalStore.test.ts                     24/24
batch3Keys.test.ts                           36/36
sermonNotesPanel.test.tsx                    13/13（组件级，见下）
verify-cooperation-identity.mjs              15/15
verify-personal-records-identity.mjs         20/20
verify-custom-groups / verify-course-flow    27/27 · 74/74
vitest 428/428 · tsc 干净 · build PASS
```

讲道笔记原本内联在 `VoiceRoomOverlay` 里（那个文件牵着麦克风与传输通道，
本地起不来）。把它抽成 `SermonNotesPanel`（**同一段 JSX、同一段逻辑，
行为没改**）之后可以组件级验证：同房不同人、防抖中换身份那次保存作废、
写失败提示与恢复。**这不是真实语音房验收** —— 真房那段仍记在未验证项里。

### 仍未处理

设备偏好那一类（语言、推送开关、各种「不再提示」标记）按判据**应当**
留在设备级，不改。若将来产品口径变了再说。
