...
## #37 聊天「+」里三个入口点开是空白面板 — `OPEN`（需要产品口径）

```
status:    OPEN
severity:  medium（用户可见：点了之后是一张空白卡片，只有关闭键）
owner:     unassigned
phase:     发现于 2026-09-12（#36 同一流程）
```
...
**为什么本轮没顺手做掉**：这三项各自要什么语义是产品决定，不是补个 UI ——

```
发布代祷   代祷在祷告室与校友圈都已经存在，聊天里的代祷是同一回事还是
           另一种东西？要不要进祷告墙？
学术提问   跟「向 AI 牧者提问」是一回事吗？要不要留档给老师？
递交作业   涉及文件上传与批改，后端没有对应端点
```

凭空发明会造出第二套语义。**也没有删掉这三个菜单项** —— 删一个用户看得见
的入口同样是产品决定。等口径。
