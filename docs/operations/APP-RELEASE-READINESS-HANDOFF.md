...
| **D. 记忆状态与事实不符（仅 #12）** | #12（见 §4） | 本文已给出证据，建议按证据更新记忆 |
| **E. 刻意保留的债，仍然有效** | #RB-25 · #RB-27（见 §4） | **不得因「当前无人使用」而判为已关闭** |
...
## 4. 三条需要区分对待的记忆条目（附本轮证据）

三条在 `OPEN_ISSUES.md` 里都写着 OPEN，但**性质完全不同**，不能并列处理。
**本文只给证据，未改任何一条的状态。**

### 4.1 只有 #12 是「记忆写的与代码事实不符」
...
这一条的修复**已合入并有回归覆盖**，记忆里的「main 未合入」是过期描述。

### 4.2 #RB-25 / #RB-27 是仍然有效的刻意债 —— 不是已修项

> ⚠️ **更正（Supervisor 2026-09-11 复核意见，已接受）**：本文早先版本把这两条与
> #12 一起归在「已过期状态」下，这个归类是错的。它们描述的是**当前依然成立的
> 刻意保留的债**，而不是已被证明修复的缺陷。
> **「当前没有使用方」恰恰是这两条债的内容本身，不是它们可以关闭的理由。**

| 条目 | 债的内容 | 本轮实测（证实债仍在，而非证实已修） |
|---|---|---|
| **#RB-25** `requireAppSecret` 保留了一个零路由使用的鉴权中间件 | 死代码型鉴权面：定义存在、无人调用，后续若被误用会绕过现行 Supabase 鉴权 | `backend/src` 中除定义处外**引用数 = 0** —— 与债的描述一致，**债仍在** |
| **#RB-27** `refresh_jti` 表已无写入方但仍保留表与历史数据 | 结构性残留：schema 与 111 行历史数据留在 canonical 库中，且当前无写入语义 | 全仓**没有任何 INSERT**，唯一引用是 `db.ts:621` 测试用 `resetDb()` 里的 DELETE；canonical 库中仍有 111 行历史数据 —— **债仍在** |

两者的正确状态是 **OPEN（刻意保留）**，处置需要单独授权（RB-25 属鉴权面清理，
RB-27 属 canonical SQLite 结构清理、与 #26 同域，而本阶段明令不动 canonical 存储）。
**本文不建议、也不授权把它们标为 CLOSED。**
...
§4 已区分「记忆过期」（仅 #12）与「仍然有效的刻意债」（#RB-25 / #RB-27）；
后者不因当前无使用方而可关闭。
