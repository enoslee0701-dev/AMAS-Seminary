...
p('- 证据强度（每维）：Likert ≥3 题且情境出现 ≥2 次 → 充分；Likert ≥2 题 → 中等；否则有限。');
...
p();
p('### 4.9 推荐引擎（规则式）');
p('- 建议尝试的侍奉 = 主倾向常见侍奉前 4 + 次要倾向前 2（去重）。措辞固定为“建议尝试”。');
p('- 建议课程 = 主倾向课程 3 门 + 次要倾向 1 门；若信仰基础整体 <60，前置「认识圣经」「基督徒生活基础」并把“信仰基础与圣经整体脉络”列为装备重点。');
p('- 若准备度整体 <50，前置“寻找一位导师或牧者，约定每月一次服事反馈”；导师反馈面向 <40 再加“邀请牧者观察一次服事并反馈”。');
p('- 成长计划：30 天（首门课前两课 + 实践 1）/ 90 天（实践 2 + 在建议侍奉中承担具体角色 + 记录 2 条服事反思）/ 6 个月（邀请导师正式反馈 + 重做 Christian Profile 对比变化）。');
p();
...
p('- **建议尝试的侍奉**卡：来自 Top 3 倾向的常见侍奉，可一键提交事奉申请。');
