...
p('## 1. 产品定位与铁律');
p();
p('**产品名**：AMAS Christian Profile（基督徒成长与事奉画像）。App 内入口叫「定制化神学」，**界面显示名统一为「信仰成长档案」**（2026-09 与官网口径对齐，英文名仅保留在装饰性字头与本规范中）。');
p();
...
p('| Level 2 | 信仰成长档案 · 完整版（App） | 84 | A 12 / B 12 / C 36 / D 12 / E 12，分 6 个阶段 | 以上 + 信仰基础、门徒生命、事奉准备度、倾向×准备度矩阵解读 |');
p();
...
p();
p('**复盘结论 → 证据极性**（P2-A.1 修正后）：');
p();
p('P2-A 曾把「未能验证」直接当成反证，这是一个语义错误。「未能验证」实际混着四种完全不同的情况：');
p('做了而且明显不合 / 只有一次机会或场合不对 / 第一次做得吃力（学习曲线）/ 根本没做成。');
p('把它们一律压成 challenge，等于用「新手第一次讲道紧张」去推翻一个人的教导倾向——');
p('方向相反，但性质等同于禁止项里的「用高倾向分数自动推断人格缺点」。因此结论拆成四档：');
p();
p('| 复盘结论 | 极性 | 强度 | 效果 |');
p('|---|---|---|---|');
p('| 验证成立 | support | 完成 ≥2 项行动为 strong，否则 moderate | 可信度升到「较高」 |');
p('| 部分成立 | support | moderate | 计为支持；第一次尝试通常如此 |');
p('| **这次没能判断** | **neutral** | weak | **只记录尝试过，不下调任何判断** |');
p('| **明显不合适** | **challenge** | moderate（自我报告封顶） | 进入冲突逻辑，见下 |');
p();
p('选「这次没能判断」时必须给出原因（机会太少 / 场合不对应 / 刚开始看不出来 / 中途有状况 / 其他），');
p('迫使系统把「没机会验证」与「验证了但不成立」分开记录。');
p();
p('**一次自我报告永远不足以推翻结论**：单条自我报告的反证只判定为 `mixed`（降一档），');
p('文案明说「单独一次还不足以改变判断，值得再验证一两次」。');
p('只有反证**重复出现（≥2 条）**，或**来自导师／同伴的观察**时，才升级为 `contradicted`（压到「较低」）。');
p();
p('措辞铁律：两者都不是「失败」。「明显不合适」说的是这个场合、这个阶段，不是这个人。');
p('旧数据里的 `not_confirmed` 语义含混，一律保守读作「这次没能判断」，**绝不追认为反证**。');
p();
...
p('1. **你的信仰成长档案**：深蓝金字头卡：组合标签（或「多元事奉组合」）、英文倾向名、一句总结（“你目前呈现较明显的「X–Y」倾向：…”）、精简版/完整版标记、结果证据强度。');
...
p();
p('### 6.2 「定制化神学」页内的呈现');
p('- **我的信仰成长档案卡**：未评估 → 两个入口（事奉倾向画像·精简版 30 题 / 信仰成长档案·完整版 84 题）；已评估 → 组合标签 + Top 3 卡图与指数 + 矩阵解读一句 + 「查看完整结果」/「完成完整版」按钮 + 右上角「重新评估」「删除结果」+ 评估时间。卡内下半部分是 12 项倾向画廊（横向滑动，点卡进入倾向说明页）。');
p('- **倾向说明页**：卡图 + 一句定义 + 常见优势 + 盲点 + 侍奉 + 装备方向；已评估时显示“你在这一维度的当前指数与排名”；上一个/下一个与 12 缩略图切换。');
p('- **我的成长档案完成度**：背景与处境 15% + 信仰基础与装备画像（九维）35% + 信仰成长档案 20% + 学习佐证 15% + 服事验证 15%。');
...
p();
p('### 6.3 网页版（Level 0）');
p('- 着陆页：精简入口条 + 12 项倾向画廊（可看说明，优势可见，盲点/侍奉/装备方向锁定并引导 App）+ 对比卡（网页快速探索 vs App 信仰成长档案）。');
...
p();
p('**结果页免责声明**：AMAS 信仰成长档案旨在帮助基督徒认识自己的信仰基础、成长实践、事奉倾向和装备需要。评估结果属于发展性参考，不用于衡量个人属灵价值，也不替代圣经、祷告、教会群体、牧者或导师的长期辨识。事奉方向应在真实生命、群体关系与持续实践中进一步确认。');
p();
p(`**倾向声明**：${ARCH_DISCLAIMER}`);
p();
p('**Level 0 免责声明**：这是基于少量问题形成的初步探索结果，不代表完整信仰成长档案。完成标准评估后，系统将使用更多行为、情境和实践信息生成更稳定的成长档案。');
p();
p('---');
p('## 9. 自检清单（每次改动评估系统前）');
p();
